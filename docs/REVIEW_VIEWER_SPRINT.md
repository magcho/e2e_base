...
画面の基本動詞は **選ぶ → 照合する**。人間の合否・進捗集計は Viewer に持たず、**Git / PR で担保**する。  
ユーザー向け用語は [REVIEW_GLOSSARY.md](./REVIEW_GLOSSARY.md) に統一する。

- 上部にレビューの問いを明示
- 状態を分離: 実行結果（合格/不合格） / 要確認
- 選択中の検査意図に対応する実行手順を中央でハイライトし、位置へスクロールする（一覧の差し替えはしない）
- カラム名: 検査意図 / 実行手順 / 証跡
...
未実装（次）: Binding差分時の前回／今回要素サムネイル比較
