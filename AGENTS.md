...
| 検査手順の提出（エージェント用） | [skills/submitting-inspection-after-impl/SKILL.md](skills/submitting-inspection-after-impl/SKILL.md) |

アプリ実装後に再実行可能な `.playbook` を提出するときは、NL→IR Translator ではなく **[submitting-inspection-after-impl](skills/submitting-inspection-after-impl/SKILL.md)** に従う（内部で write / run / qualify の各 Skill を組み合わせる）。形式チェックは既存パーサ、意味確認は Runtime + Report（Qualification）とする。
...
- `docs/` — 設計・計画一式
- `skills/` — エージェント用 Skill（単一責務 + ユースケース組み合わせ）
  - primitive: `writing-playbook-dsl` / `running-playbook` / `qualifying-playbook`
  - usecase: `submitting-inspection-after-impl` / `revising-playbook-for-requalification`
